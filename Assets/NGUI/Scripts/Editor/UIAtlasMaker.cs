...
public class UIAtlasMaker : EditorWindow
{
    ...
    static public void CreateAltas(string parentPath,string autoAtlasName ,List<Texture> textures ,bool keepSprites)
    {
        //var path = EditorUtility.SaveFilePanelInProject("Save As", autoAtlasName, "asset", "Save atlas as...", parentPath);
        var path = parentPath + autoAtlasName;
        if (!string.IsNullOrEmpty(path))
        {
            NGUISettings.currentPath = System.IO.Path.GetDirectoryName(path);
            var asset = AssetDatabase.LoadAssetAtPath<NGUIAtlas>(path);
